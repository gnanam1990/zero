...
func TestSuggestionOverlayCapsRowsWithoutMoreText(t *testing.T) {
	...
	// whenever a command is inserted above it — /plans pushed /ps out — so what
	// this pins is the window's start and size, not those two names.
	if !strings.Contains(plain, "│ ❯ provider") || !strings.Contains(plain, "│   permissions") {
